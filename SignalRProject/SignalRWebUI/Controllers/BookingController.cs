...
namespace SignalRWebUI.Controllers
{
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responMessage = await client.GetAsync("https://localhost:7120/api/Booking");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7120/api/Booking/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateBooking(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7120/api/Booking/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateBooking(UpdateBookingDto updateBookingDto)
        {
            if (TempData["id"] != null)
            {
                ...
                {
                    updateBookingDto.HtmlEncodeProperties();
                    var client = _httpClientFactory.CreateClient();
                    var jsonData = JsonConvert.SerializeObject(updateBookingDto);
                    StringContent strContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                    var responseMessage = await client.PutAsync("https://localhost:7120/api/Booking", strContent);
                    if (responseMessage.IsSuccessStatusCode)
                    {
                        TempData["id"] = null;
                        return RedirectToAction("Index");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.GetAsync($"https://localhost:7120/api/Booking/BookingStatusApprov/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.GetAsync($"https://localhost:7120/api/Booking/BookingStatusCancel/{id}");
